#!/usr/bin/env python3
"""Forward test v6 — Round 5: Multi-day Reversal + Post-crash Recovery.

Target: beat 沪深300 by >= 5 percentage points.

Changes from Round 4:
  1. Multi-day reversal: 2d cumulative > 2.5% or 3d > 3% → bump regime
  2. Single-day reversal lowered: > 1.5% (was 2.0%)
  3. Post-crash recovery: after crash stop, next rebalance bumps regime 1 level
  4. Minimum 3 sectors always (better diversification)
  5. Sector concentration limit: max 50% of invested amount per sector
"""
...
REGIME_MA_PERIOD = 10
REVERSAL_DAY_SINGLE = 1.5
REVERSAL_2D_CUMULATIVE = 2.5
REVERSAL_3D_CUMULATIVE = 3.0
SECTOR_CONCENTRATION_CAP = 0.50  # max 50% of invested amount per sector
...
def market_regime(index, date) -> str:
    """Classify market regime using MA10."""
...
def check_reversal(index, day_idx, trading_days) -> tuple[bool, int]:
    """Multi-day reversal detection. Returns (is_reversal, bump_levels)."""
    idx_pct_single = 0.0
    if day_idx >= 0:
        row = index[index["date"] == trading_days[day_idx]]
        idx_pct_single = float(row["pct_chg"].iloc[0]) if not row.empty else 0.0

    # Single day > 1.5%
    if idx_pct_single > REVERSAL_DAY_SINGLE:
        return True, 1

    # 2-day cumulative > 2.5%
    if day_idx >= 1:
        row_prev = index[index["date"] == trading_days[day_idx - 1]]
        pct_prev = float(row_prev["pct_chg"].iloc[0]) if not row_prev.empty else 0.0
        cum2 = pct_prev + idx_pct_single
        if cum2 > REVERSAL_2D_CUMULATIVE:
            return True, 2 if cum2 > 3.5 else 1

    # 3-day cumulative > 3.0%
    if day_idx >= 2:
        row_d2 = index[index["date"] == trading_days[day_idx - 2]]
        row_d1 = index[index["date"] == trading_days[day_idx - 1]]
        pct_d2 = float(row_d2["pct_chg"].iloc[0]) if not row_d2.empty else 0.0
        pct_d1 = float(row_d1["pct_chg"].iloc[0]) if not row_d1.empty else 0.0
        cum3 = pct_d2 + pct_d1 + idx_pct_single
        if cum3 > REVERSAL_3D_CUMULATIVE:
            return True, 2 if cum3 > 5.0 else 1

    return False, 0


class Round5Engine:
    """Forward test with multi-day reversal + post-crash recovery."""
    ...
    def __init__(self, capital: int = INITIAL_CAPITAL):
        ...
        self.post_crash_recovery = False
        self.crash_count = 0
    ...
    def run(self, sectors: dict, index: pd.DataFrame,
        ...
        for day_idx, day in enumerate(trading_days):
            ...
            if had_crash:
                self.post_crash_recovery = True
                self.force_rebalance = True

            # ---- Regime detection + reversal check ----
            regime = market_regime(index, day)
            is_reversal, bump_levels = check_reversal(index, day_idx, trading_days)

            if is_reversal and regime in ("cash", "defensive"):
                old_regime = regime
                regime = bump_regime(regime, bump_levels)
                self.force_rebalance = True
                reversal_events.append(
                    f"  {day_str}: 反弹信号(level={bump_levels}) → {old_regime}→{regime}, 强制调仓")

            # Post-crash recovery: bump regime for next rebalance
            if self.post_crash_recovery and self.force_rebalance and not is_reversal:
                old_regime = regime
                regime = bump_regime(regime, 1)
                reversal_events.append(
                    f"  {day_str}: 灾后恢复模式 → {old_regime}→{regime}")
        ...
        if reversal_events:
            print(f"\n{'='*60}")
            print(f"反弹/恢复事件 ({len(reversal_events)} 次)")
            print(f"{'='*60}")
    ...
    def _compute_targets(self, sectors, index, date, pos_cap):
        ...
        # Always try for 3 sectors (better diversification)
        effective_top_n = 3

        # Collect ETF-qualified sectors
        qualified = []
        for _, row in scores_df.iterrows():
            ...
            if len(qualified) >= effective_top_n:
                break
        ...
        # Raw rank-weighted allocation, then apply concentration cap
        targets = []
        rank_sum = sum(range(1, n + 1))
        max_per_sector = round(invest_amount * SECTOR_CONCENTRATION_CAP)

        for q in qualified:
            rel_weight = (n - q["rank"] + 1) / rank_sum
            amount = round(invest_amount * rel_weight)
            if max_per_sector > 0:
                amount = min(amount, max_per_sector)
            if amount > 0:
    ...
    def _summary(self, start_date, end_date, index):
        ...
        print(f"\n{'='*80}")
        print(f"Round 5: Multi-day Reversal + Post-crash Recovery — {start_date} → {end_date}")
        print(f"Regime MA: {REGIME_MA_PERIOD}d | Reversal: single>{REVERSAL_DAY_SINGLE}% 2d>{REVERSAL_2D_CUMULATIVE}% 3d>{REVERSAL_3D_CUMULATIVE}%")
        print(f"Caps: {REGIME_CAPS} | Sector cap: {SECTOR_CONCENTRATION_CAP*100:.0f}% | Min sectors: 3")
        ...
        print(f"Max drawdown: {max_dd:.2f}% | Crash events: {self.crash_count}")
        print(f"{'='*80}")
        return df
...
def main():
    parser = argparse.ArgumentParser(description="Round 5: Multi-day reversal + post-crash recovery")
    ...
    print(f"\n>>> Round 5: Multi-day Reversal + Post-crash Recovery <<<")
    print(f"  Reversal: single>{REVERSAL_DAY_SINGLE}% 2d>{REVERSAL_2D_CUMULATIVE}% 3d>{REVERSAL_3D_CUMULATIVE}%")
    print(f"  Post-crash recovery: ON | Min sectors: 3 | Sector cap: {SECTOR_CONCENTRATION_CAP*100:.0f}%")

    engine = Round5Engine(capital=args.capital)
    df = engine.run(sectors, index_data, args.start, args.end)
    print_daily_report(df)

    from config.settings import RESULTS_DIR
    out_path = RESULTS_DIR / f"forward_{args.start}_{args.end}_round5.csv"
