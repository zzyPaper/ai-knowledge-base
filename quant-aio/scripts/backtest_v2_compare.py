...
def run_v2_backtest(start: str, end: str) -> dict:
    strategy = V2ThreeFactor()
    ...
    # 从 daily_values 计算夏普和最大回撤
    if result.daily_values:
        df = pd.DataFrame(result.daily_values)
        df["date"] = pd.to_datetime(df["date"])
        df = df.set_index("date").sort_index()
        nav = df["total_value"] / result.initial_cash  # 归一化为净值
